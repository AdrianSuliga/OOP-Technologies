...
public class Student {
    ...
    public static Optional<Student> findByIndexNumber(final int indexNumber) {
        try {
            ResultSet resultSet = QueryExecutor.read("SELECT * FROM student WHERE index_number=?", indexNumber);
            return Optional.of(new Student(resultSet.getInt(Columns.ID),
    ...
    public Map<Course, Float> createReport() {
        String sql = "SELECT c.id, c.name, AVG(g.grade) AS avg_grade FROM grade AS g INNER JOIN course as c ON c.id = g.course_id WHERE g.student_id = ? GROUP BY c.id ORDER BY g.course_id";
